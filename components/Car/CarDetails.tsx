    ...
    <Transition appear show={isOpen} as={Fragment}>
        <Dialog as="div" className="relative z-10" onClose={closeModal}>
          ...
          <div className='fixed inset-0 overflow-y-auto'> 
              ...
              >
                <Dialog.Panel className="relative w-full max-w-lg max-h-[90vh] p-6 
                overflow-y-auto transform rounded-2xl bg-white
                text-left shadow-2xl transition-all flex flex-col gap-5">
                  <button type="button"
                  className='absolute top-2 right-2 z-10 w-fit p-2 bg-yellow-400 rounded-full'
                  onClick={closeModal}
                  >
                  ...
                  <div className='flex-1 flex flex-col gap-3'>
                      ...
                      <div className='flex gap-3'>
                          ...
                          <div className='flex-1 relative w-full h-24 bg-white rounded-lg'>
                          <Image src={generateCarImageUrl(car, '48')} alt="car model" fill priority className="object-contain"/>
                          </div>
                      </div>
                  </div>
                      ...
                      <div className='mt-3 flex flex-wrap gap-3'>
                          {Object.entries(car).map(([key, value]) => (
                            <div className='flex justify-between gap-5 w-full text-right' key={key}>
                              ...
                              <p className='text-black font-semibold capitalize'>{value}</p>
                            </div>
                          ))}
                      </div>
                  </div>
